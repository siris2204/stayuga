...
- Name: filters.spec.ts >> Property filters >> Selecting check-in then check-out adds both to URL on Apply
- Location: tests\e2e\filters.spec.ts:72:7
...
Call log:
  - waiting for locator('.grid.grid-cols-7 button:not([disabled])').first()
